...
public class GraphicsDisplay extends JPanel {
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            if (point[0] >= viewport[0][0] && point[1] <= viewport[0][1] && point[0] <= viewport[1][0]) {
                ...
                canvas.setColor(Color.BLACK);
                if (Math.abs(II - Math.round(II))< 0.1) {
    ...
    protected void paintGreed(Graphics2D canvas){
        ...
        int interaro = 0;
        for(double step = minY; step < maxY; step += (maxY - minY) / 100, interaro++){
            if(interaro % 5 == 0){
                canvas.draw(new Line2D.Double(this.xyToPoint(-0.125, step), this.xyToPoint(0.125, step)));
            }
            canvas.draw(new Line2D.Double(this.xyToPoint(-0.035, step), this.xyToPoint(0.035, step)));
        }
        interaro = 0;
        for(double step = minX; step < maxX; step += (maxX - minX) / 100, interaro++){
            if(interaro % 5 == 0){
                canvas.draw(new Line2D.Double(this.xyToPoint(step,-0.125 ), this.xyToPoint(step, 0.125)));
            }
            canvas.draw(new Line2D.Double(this.xyToPoint(step, -0.035), this.xyToPoint(step, 0.035)));
        }
//        for (double pos = this.viewport[0][0], step = (this.viewport[1][0] - this.viewport[0][0]) / 100.0; pos < this.viewport[1][0]; pos += step) {
//            canvas.draw(new Line2D.Double(this.xyToPoint(step, -0.5), this.xyToPoint(step, 0.5)));
//        }
...
//        for (double pos = this.viewport[1][1], step = (this.viewport[0][1] - this.viewport[1][1]) / 100.0; pos < this.viewport[0][1]; pos += step) {
//            canvas.draw(new Line2D.Double(this.xyToPoint(step, -0.5), this.xyToPoint(step, 0.5)));
//        }
    }
